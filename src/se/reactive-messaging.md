...
All [Messaging connectors](#messaging_connector) in Helidon are made to
be universally usable by Helidon MP and SE.

## Channel

A channel is a named pair of `Publisher` and `Subscriber`. Channels can
be connected together by [processors](#processor). Registering a
`Publisher` or `Subscriber` for a channel can be done by Messaging API,
or configured implicitly using registered [connectors](#connectors) to
...
messaging, it is able to connect two [channels](#channel) to one
reactive stream.
...
image, for easy testing: https://github.com/helidon-io/helidon-examples/tree/helidon-4.x/examples/messaging
...
- [Implicit Helidon Configuration for Kafka Connector](#implicit_helidon_config_for_kafka_connector)
